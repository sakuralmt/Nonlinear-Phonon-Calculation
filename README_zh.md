...
```mermaid
flowchart TB
    A["外部输入树<br/>structure.cif + pseudos + system.json"] --> B["npc / tui<br/>统一操作入口"]
    B --> C["创建或复用 run root"]
    C --> D["stage1<br/>QE phonon frontend"]
    D --> E["contracts/stage1.manifest.json"]
    E --> F["stage2<br/>MLFF 筛选<br/>默认 GPTFF v2"]
    F --> G["contracts/stage2.manifest.json"]
    G --> H["stage3<br/>QE top5 recheck"]
    H --> I["contracts/stage3.manifest.json"]
    B --> J["只读状态检查<br/>./npc --status"]
    B --> K["跨机 handoff<br/>export 或 import bundle"]
    E --> K
    G --> K
```
...
- `qe_phonon_stage1_server_bundle/`
  - `stage1` 声子前端与收敛性工具
- `mlff_modepair_workflow/`
  - 面向 GPTFF、CHGNet 与 MatterSim 后端的 stage2 MLFF 筛选逻辑
- `qe_modepair_handoff_workflow/`
  - `stage3` QE 准备与回收逻辑
