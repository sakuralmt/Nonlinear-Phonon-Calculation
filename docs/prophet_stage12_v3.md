...
./npc --stage stage1 --stage1-backend prophet \
  --stage2-model prophet_oame_mbd --system-dir /path/to/mos2-system \
  ...
  --q-grid-n 6 --fd-step 0.01 --stage1-device cuda

./npc --stage stage2 --stage2-model prophet_oame_mbd \
  --run-root /path/to/runs/mos2/shared_dft --stage2-device cuda
```

Repeat for WSe₂ and for separate `model_relaxed` run roots. Stage1 uses ±0.01 Å force differences on the commensurate supercell and repeats the representative q frequencies with ±0.005 Å. It stores the full force constants, every q frequency/eigenvector, q-orbit membership, acoustic and Hermiticity diagnostics. Historical QE modes are an **offline validation source**, not the new mode-pair source. To compare a completed Stage1 run with archived QE files, use `python scripts/compare_prophet_stage1_qe.py --phonon-dataset ... --matdyn-input ... --qe-eig ... --output ...`.
...
Old Stage1/Stage2 v2 results remain read-only. In particular, old K-point real-mode amplitudes use a different normalization, so their coefficients cannot enter a v3 ranking. Refit an old raw energy grid only when its actual displaced coordinates can be reconstructed. Map QE and Prophet modes by overlap on the **same structure**; compare near-degenerate eigenspaces as subspaces. If this mapping or the displacement definition is unreliable, label the coupling **not directly comparable**. No agreement of Prophet third-order coefficients with old QE-mode DFT coefficients is used as a release gate.
