...
def write_relaxed_structure_from_qe_template(source: Path, destination: Path, atoms) -> None:
    ...
    for offset, (symbol, position, flags) in enumerate(
        zip(atoms.get_chemical_symbols(), atoms.get_scaled_positions(), fixed_flags), start=1
    ):
        lines[atom_header + offset] = f"{symbol}  " + "  ".join(f"{value:.15f}" for value in position) + ("  " + " ".join(flags) if flags else "")
    destination.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.NamedTemporaryFile(mode="w", dir=destination.parent, prefix=".relaxed-", delete=False) as handle:
        temporary = Path(handle.name)
        handle.write("\n".join(lines) + "\n")
    try:
        recovered = load_atoms_from_qe(temporary)
        if (not np.allclose(recovered.cell.array, atoms.cell.array, atol=1e-10)
...
def relax_structure_with_calculator(source: Path, output_dir: Path, calculator,
                                    model_meta: dict, backend: str):
    ...
    if summary_path.exists():
        ...
        if (summary.get("relaxation_protocol_version") != RELAXATION_PROTOCOL_VERSION
                or summary.get("backend") != backend
                or summary.get("source_structure_sha256") != source_hash
                or any(previous_model.get(key) != model_meta.get(key) for key in
                       ("checkpoint_sha256", "verified_source_tree_sha256", "actual_source_commit"))
                or not destination.is_file()
                or summary.get("optimized_structure_sha256") != sha256_file(destination)):
            raise ValueError(f"Existing model relaxation differs from requested inputs: {summary_path}")
        return destination, summary
    if destination.exists():
        raise ValueError(f"Relaxed structure exists without a matching summary: {destination}")
    ...
    def evaluate_scale(scale: float):
        ...
        raw_force = float(np.max(np.linalg.norm(atoms.get_forces(apply_constraint=False), axis=1)))
        trials.append({"scale": float(scale), "energy_ev": energy,
                       "max_force_ev_per_A": constrained_force,
                       "max_raw_force_ev_per_A": raw_force, "converged": converged})
        if not converged or not np.isfinite(energy):
            return float("inf")
        structures.append(atoms)
        return energy

    search = minimize_scalar(evaluate_scale, bounds=(0.94, 1.06), method="bounded",
                             options={"xatol": 1e-3, "maxiter": 14})
    if not search.success or not structures:
        raise RuntimeError(f"{backend} structural relaxation failed; inspect trial logs in {output_dir}")
    best_index = min((i for i, row in enumerate(trials) if row["converged"]),
    ...
    if best_scale <= 0.941 or best_scale >= 1.059:
        raise RuntimeError(f"{backend} in-plane lattice optimum reached the search bound")
    write_relaxed_structure_from_qe_template(source, destination, best)
    summary = {
        "kind": f"{backend}_model_relaxation", "backend": backend,
        "relaxation_protocol_version": RELAXATION_PROTOCOL_VERSION,
        "source_structure": str(source), "source_structure_sha256": source_hash,
        "optimized_structure": str(destination),
        "optimized_structure_sha256": sha256_file(destination),
        "model": model_meta, "best_inplane_scale": best_scale,
        "best_energy_ev": float(best.get_potential_energy()),
        "vacuum_c_fixed": True, "qe_relaxation_flags_preserved": True,
        "bfgs_fmax_ev_per_A": 0.02, "bfgs_max_steps": 200,
        "inplane_scale_bounds": [0.94, 1.06], "trials": trials,
    }
    with tempfile.NamedTemporaryFile(mode="w", dir=output_dir, prefix=".relax-summary-", delete=False) as handle:
        temporary = Path(handle.name)
