...
\input{tables/geometry.tex}\end{table}
...
\begin{figure}[!htbp]\centering\includegraphics[width=0.88\textwidth]{figures/qgrid_frequency_error.pdf}
...
\begin{figure}[!htbp]\centering\includegraphics[width=0.92\textwidth]{figures/cubic_channels.pdf}
...
\begin{figure}[!htbp]\centering\includegraphics[width=0.92\textwidth]{figures/quartic_channels.pdf}
...
\input{tables/rank.tex}\end{table}
\FloatBarrier
...
\input{tables/m6_contrast.tex}\end{table}
\begin{figure}[!htbp]\centering\includegraphics[width=0.93\textwidth]{figures/ws2_m6_same_geometry_pes.pdf}
...
\input{tables/window.tex}\end{table}
...
\begin{figure}[!htbp]\centering\includegraphics[width=0.82\textwidth]{figures/displacement_density.pdf}
\caption{将窗口变化与固定窗口下的采样密度变化分开展示。蓝色包含 $|Q|\leq1\to2$ 的窗口效应；橙色只改变 $|Q|\leq2$ 内的步长。}\label{fig:displacement_density}\end{figure}

这说明\emph{三个最强通道在宽窗口内}对一次密度加倍不敏感；尚不足以证明局部四阶 Taylor 导数已严格收敛。特别是其他十二对只有中心 5$\times$5，而中心 $|Q_i|\leq1$ 的 9$\times$9（步长 0.25）并未计算。WS$_2$ M6 的 $Q=0.5$、1 偶混合对比确认变号不是单个步长或拟合算法产生，但同样不能取代该通道的固定窗口密度测试。下一步若追求局部四阶精度，应优先在 WS$_2$ M6 与每材料首通道补中心 9$\times$9，然后同时检查 $|Q_i|\leq0.5$ 的缩窗拟合。

\begin{table}[!htbp]\centering\small\caption{三种材料的首通道 18 点电子收敛先导结果：六点三阶估计的相对变化，均满足预设 2\% 停机门槛。}\label{tab:pilots}
\input{tables/pilots.tex}\end{table}

\FloatBarrier

\section{成本、适用性与下一轮实验}

三个材料分别完成 193 个 QE 单点：五对中心 25 点、最强对补齐 81 点，以及先导点，总计 579 点。按 Slurm 计量，PBE 势能面单点合计 104.09 节点时，未包括三次 DFPT、控制器或 MLFF 早期筛选。12 原子的 M/K 模式单点中位数约 5.8--6.7 分钟，27 原子的 L 模式约 22--27 分钟（表~\ref{tab:cost}；图~\ref{fig:compute_cost}）。因此先以便宜的 MLFF 在所有候选上排序、仅对强耦合且可信的模式做 DFT，是有实际计算意义的；本报告没有测定本轮各 Stage1/Stage2 的完整端到端墙钟，不能从此表推算 MLFF 的精确加速比。

\begin{table}[!htbp]\centering\small\caption{QE PBE PES 单点的实际 Slurm 成本。节点时按单点作业计，不含 DFPT 与控制器；中位秒按相应原子数分组。}\label{tab:cost}
\input{tables/cost.tex}\end{table}
...
\end{itemize}
旧模型参考只读取 Git 稳定仓库的验收数据目录，没有修改稳定代码。报表输入文件的 SHA256 保存在本目录的 \texttt{sources.json}。运行 \texttt{generate\_assets.py} 可重建图表，随后用 XeLaTeX 编译本文件。主比较按五对物理通道对应，旧 WSe$_2$ 未恢复的四阶值如实显示为缺失。
