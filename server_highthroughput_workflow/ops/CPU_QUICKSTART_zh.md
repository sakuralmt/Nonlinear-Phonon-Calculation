...
```bash
bash server_highthroughput_workflow/ops/bootstrap_server_env.sh
python server_highthroughput_workflow/ops/assess_chgnet_env.py
...
- 后续 `run_pair_screening_optimized.py` 和 `stage23_pipeline.py`
  会自动优先使用这份固定配置
...
```bash
python server_highthroughput_workflow/stage23_pipeline.py \
  --scheduler auto \
  --runtime-profile medium
```
